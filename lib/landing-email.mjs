...
export async function sendSubmissionEmail(
  ...
  const message = kind === 'waitlist'
    ? waitlistEmail({ email, position, submittedAt })
    : suggestionEmail({ thought, email, submittedAt });
...
export function suggestionAcknowledgementEmail({ thought }) {
  return {
    subject: 'We got your idea — The Fourth Place',
    text: `We got it. Every thought shapes what comes next.\n\nYour thought:\n${thought}`,
    html: `
      <div style="font-family:monospace;padding:32px;background:#0a0a0f;color:#fff;max-width:520px">
        ...
        <div style="border-left:1px solid rgba(255,255,255,0.15);padding-left:16px;margin-bottom:32px">
          <p style="font-size:13px;color:rgba(255,255,255,0.35);line-height:1.7;margin:0;white-space:pre-wrap">${escapeHtml(thought)}</p>
        </div>
      </div>
    `,
  };
